...
<Tabs items={["yarn", "npm", "pnpm"]}>
  <Tabs.Tab>```bash copy yarn add @village-wellth/next-admin@latest ```</Tabs.Tab>
  <Tabs.Tab>
    ```bash copy npm install -S @village-wellth/next-admin@latest ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```bash copy pnpm install -S @village-wellth/next-admin@latest ```
  </Tabs.Tab>
</Tabs>
...
Update your Next-Admin page to use the **new** `getNextAdminProps` **function** (more details [here](/docs/api/get-next-admin-props-function)).
  ...
  <Tabs items={['App Router', 'Pages Router']}>
    ...
    ```ts copy filename="app/admin/[[...nextadmin]]/page.tsx"
      import { NextAdmin, PageProps } from '@village-wellth/next-admin';
    ...
    <Tabs.Tab>
    ```ts copy filename="pages/api/admin/[[...nextadmin]].ts"
      import { AdminComponentProps, NextAdmin } from '@village-wellth/next-admin';
