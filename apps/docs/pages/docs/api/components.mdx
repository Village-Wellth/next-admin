...
Next-Admin exports a set of UI components which are, for most of those, extending [Radix UI primitives](https://www.radix-ui.com/primitives). These components are available through the `@village-wellth/next-admin/components` import.
...
The inputs used across next-admin are available to be used in custom pages. These components are available through the `@village-wellth/next-admin/inputs` import.
Most expect to be wrapped within a `FormDataProvider`:

```tsx
// pages/admin/coupons/batch-upload.tsx
import { FormDataProvider, FileWidget, useFormData } from "@village-wellth/next-admin/inputs";
